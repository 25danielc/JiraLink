...
// plain text. The single field that matters — "Changelog Entry" — is converted
// back to verbatim Markdown by lib/adf.js at the point of use (collect.js).
// Everything else we read (summary, labels, dates, issuetype) is already scalar.
...
// --- Diagnostics ---------------------------------------------------------
// These exist purely to explain a zero-result run: they confirm that jiraBase +
// the credentials resolve to the site you expect, and that the project key is
// actually visible to this token. Not used on the happy path.

// The account the token authenticates as. Proves JIRA_BASE + JIRA_EMAIL +
// JIRA_TOKEN are coherent (a wrong base or bad token fails loudly instead).
export async function getCurrentUser() {
  return get("/rest/api/3/myself");
}

// Project keys this token can see (first page — plenty for a diagnostic). If the
// configured key isn't in here, the key is wrong or you're pointed at the wrong
// Atlassian site.
export async function listVisibleProjects() {
  const data = await get("/rest/api/3/project/search", { maxResults: 100 });
  return (data.values || []).map((p) => p.key);
}
